...
const DSN = "https://pk_live_abc123@traceforge.example.com/project/tf_12345abc"

describe("parseDsn", () => {
  it("extracts the ingest URL, project id and public key", () => {
    expect(parseDsn(DSN)).toEqual({
      ingestUrl: "https://traceforge.example.com/api/v1/events",
  ...
  it("keeps a self-hosted base path", () => {
    expect(parseDsn("http://pk@localhost:4000/pulse/project/tf_12345abc")?.ingestUrl).toBe(
  ...
  it.each([
    "not a url",
    "https://traceforge.example.com/project/tf_12345abc", // no public key
    "https://pk@traceforge.example.com/project/12345", // bad project id
    "ftp://pk@traceforge.example.com/project/tf_12345abc",
  ])("rejects %s", (dsn) => {
